...
const SVMModel = () => {
    const [formData, setFormData] = useState({
        ...
        heartRate: 75,
        glucose: 90,
        model_type: "svm" // Automatically select SVM model
    });
        ...
        <div>
            <div className="flex justify-center items-center min-h-screen bg-gray-900 text-white p-8">
                ...
                >
                    {/* Left Side: Questions */}
                    <div className="w-1/2">
                        <h2 className="text-3xl font-bold text-center text-blue-400 mb-6">SVM Model</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {[
                                { label: "Systolic Blood Pressure", name: "sysBP", min: 90, max: 180 },
                                { label: "Diastolic Blood Pressure", name: "diaBP", min: 60, max: 120 },
                                { label: "BMI (Body Mass Index)", name: "BMI", min: 15, max: 40 },
                                { label: "Heart Rate (BPM)", name: "heartRate", min: 40, max: 150 },
                                { label: "Glucose Level", name: "glucose", min: 50, max: 300 }
                    ...
                    <div className="w-1/3 flex justify-end">
                        {result && (
                            <motion.div
                                className={`p-6 rounded-lg shadow-lg transition-all duration-500 text-white border-2 ${result.prediction === 1 ? "border-red-500 bg-red-500/10" : "border-green-500 bg-green-500/10"}`}
                                initial={{ opacity: 0, y: 20 }}
                                animate={{ opacity: 1, y: 0 }}
                            >
                                <h3 className="text-xl font-bold">{result.model} Prediction</h3>
                                <p className="mt-2"><strong>Probability:</strong> {result.probability != null ? `${Math.round(result.probability * 100)}%` : "N/A"}</p>
